...
task SamToFastqAndBwaMem {
  ...
  String output_bam_basename=basename(in_ubam, "unmapped.bam")
  String picard_path
...
task MergeBamAlignment {
  ...
  String bwa_path
  String gatk4_path
  String out_bam_basename=basename(unmapped_bam, ".bam")
  String bwa_commandline=bwa_path + " mem -K 100000000 -p -v 3 -t 10 -Y $bash_ref_fasta"
...
task SamToFastqAndBwaMem {
  ...
  String bwa_path  
  String bwa_commandline=bwa_path + " mem -K 100000000 -p -v 3 -t 10 -Y $bash_ref_fasta"
  String output_bam_basename=basename(in_ubam, ".bam")
  String picard_path
  String samtools_path
...
task MergeBamAlignment {
  ...
  command <<<
   # set the bash variable needed for the command-line
   bash_ref_fa=${ref_fasta}
   ${gatk_path} -Xmx5g MergeBamAlignment \
     --VALIDATION_STRINGENCY=SILENT \
     --EXPECTED_ORIENTATIONS=FR \
     --ATTRIBUTES_TO_RETAIN=X0 \
     --ALIGNED_BAM=${aligned_bam} \
...
task MarkDuplicates {
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  command <<<
    ${gatk_path} -Xmx5g MarkDuplicates \
